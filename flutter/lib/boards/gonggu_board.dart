...
import 'package:usdh/maps/gonggu_map.dart';
import 'package:usdh/function/chip.dart';
import 'dart:async';

late GongguWriteState pageState;
late GongguListState pageState2;
late GongguShowState pageState3;
late GongguModifyState pageState4;
...
class GongguWrite extends StatefulWidget {
  @override
  GongguWriteState createState() {
    pageState = GongguWriteState();
    return pageState;
  }
}

class GongguWriteState extends State<GongguWrite> {
  ...
  TextEditingController goodsInput = TextEditingController();
                      ...
                      Padding(
                          ...
                          child: Wrap(
                            ...
                            children: [
                              ...
                              condWrap("구매물품", goodsInput, "구매 물품을 입력하세요.", "구매 물품은 필수 입력 사항입니다."),
                              Wrap(
                                spacing: 15,
                                crossAxisAlignment: WrapCrossAlignment.center,
                                children: [
  ...
  void uploadOnFS() async {
    ...
    await fs.collection('gonggu_board').doc(myInfo['nick'] + myInfo['postcount'].toString()).set({
...
class GongguList extends StatefulWidget {
  @override
  GongguListState createState() {
    pageState2 = GongguListState();
    return pageState2;
  }
}

class GongguListState extends State<GongguList> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar("공동 구매", [
        IconButton(
          icon: Image.asset('assets/images/icon/iconmap.png', width: 20, height: 20),
          onPressed: () {
            Navigator.pop(context);
            Navigator.push(context, MaterialPageRoute(builder: (context) => GongguMap()));
          },
        ),
        //새로고침 기능
        IconButton(
          ...
          onPressed: () {
            setState(() {
              colstream = FirebaseFirestore.instance.collection('gonggu_board').orderBy("write_time", descending: true).snapshots();
            });
          },
        ),
        //검색 기능 팝업
                            ...
                            TextButton(
                                onPressed: () {
                                  if (_formKey.currentState!.validate()) {
                                    if (search == "제목") {
                                      setState(() {
                                        colstream = FirebaseFirestore.instance.collection('gonggu_board').orderBy('title').startAt([searchInput.text]).endAt([searchInput.text + '\uf8ff']).snapshots();
                                      });
                                      searchInput.clear();
                                      Navigator.pop(context);
                                    } else {
                                      setState(() {
                                        colstream = FirebaseFirestore.instance.collection('gonggu_board').where('tagList', arrayContains: "#" + searchInput.text + " ").snapshots();
                                      });
                                      searchInput.clear();
                                      Navigator.pop(context);
                                    }
                            ...
                            child: Checkbox(
                              ...
                              onChanged: (val) {
                                setState(() {
                                  status = val ?? false;
                                  if (status) {
                                    colstream = FirebaseFirestore.instance.collection('gonggu_board').where('time', isGreaterThan: formatDate(DateTime.now(), [yyyy, '-', mm, '-', dd, ' ', HH, ':', nn, ':', ss])).snapshots();
                                  } else {
                                    colstream = FirebaseFirestore.instance.collection('gonggu_board').orderBy("write_time", descending: true).snapshots();
                                  }
                                });
                                        ...
                                        child: ListView.builder(
                                          ...
                                          itemBuilder: (context, index) {
                                            ...
                                            return GestureDetector(
                                                onTap: () {
                                                  setState(() {
                                                    colstream = FirebaseFirestore.instance.collection('gonggu_board').where('tagList', arrayContains: tag).snapshots();
                                                  });
                                                },
      ...
      floatingActionButton: FloatingActionButton(
          ...
          onPressed: () {
            Navigator.push(context, MaterialPageRoute(builder: (context) => GongguWrite()));
          }),
    );
  }
}
...
class GongguShow extends StatefulWidget {
  final String id;

  GongguShow({Key? key, required this.id}) : super(key: key);

  @override
  GongguShowState createState() {
    pageState3 = GongguShowState(id: id);
    return pageState3;
  }
}

class GongguShowState extends State<GongguShow> {
  final String id;

  GongguShowState({Key? key, required this.id});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: CustomAppBar("공동 구매", []),
        body: StreamBuilder(
          stream: fs.collection('gonggu_board').doc(widget.id).snapshots(),
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: GestureDetector(
                            ...
                            onTap: () async {
                              ...
                              await fs.collection('gonggu_board').doc(widget.id).delete();
                        ...
                        Container(
                          ...
                          child: GestureDetector(
                            ...
                            onTap: () async {
                              var tmp;
                              await fs.collection('gonggu_board').doc(widget.id).get().then((snap) {
                                tmp = snap.data() as Map<String, dynamic>;
                              });
                              Navigator.push(context, MaterialPageRoute(builder: (context) => GongguModify(widget.id, tmp)));
                              setState(() {});
                            },
                          ),
                        )
                                                            ...
                                                            onPressed: () async {
                                                              if (_formKey.currentState!.validate()) {
                                                                ...
                                                                if (_myApplication.contains(title)) {
                                                                  ...
                                                                  // 내 정보에 신청 정보를 기록
                                                                  await fs.collection('users').doc(myInfo['email']).collection('myApplication').doc(title).set({'where': "gonggu_board", 'isRejected': false, 'isJoined': false});
                                                                  ScaffoldMessenger.of(context).hideCurrentSnackBar();
...
class GongguModify extends StatefulWidget {
  GongguModify(this.id, this.datas);
  final String id;
  final Map<String, dynamic> datas;
  @override
  State<StatefulWidget> createState() {
    pageState4 = GongguModifyState();
    return pageState4;
  }
}

class GongguModifyState extends State<GongguModify> {
  late FirebaseProvider fp;
  final FirebaseFirestore fs = FirebaseFirestore.instance;
  ...
  late TextEditingController goodsInput;
  ...
  void initState() {
    setState(() {
      ...
      goodsInput = TextEditingController(text: widget.datas['goods']);
  ...
  void updateOnFS() async {
    var myInfo = fp.getInfo();
    await fs.collection('gonggu_board').doc(widget.id).update({
      'title': titleInput.text,
      'contents': contentInput.text,
